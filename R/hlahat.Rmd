...
alleleld_tab <- dna_alleleld %>% filter(Gene %in% LETTERS[1:3] & ld_median>0) %>% 
  ...
  mutate(degree_dna = case_when(2^abs(ld_median.dna) < 1.5 ~ "~", 
         ...
         conf_dna = ifelse(n_snps.dna < 5, "?", ""),
         conf_rna = ifelse(n_snps.rna < 5, "?", "")) %>%
  group_by(Gene) %>% 
  summarise(
...
dna_imb <- ggplot(data = NULL, aes(x = Gene, colour = factor(alleleN), label = CHROM)) +
  geom_hline(yintercept = 0) +
        ...
        labs(x = "Gene", y = "Log-fold Difference in Coverage", title = "Allelic Imbalance (DNA) of HLA Alleles",
        colour = "Allele N") +
        theme_bw() +
...
#  geom_hline(data = filter(sort.dna_alleleld, Gene %in% genes), aes(yintercept = ld_75, colour = factor(CHROM), label = CHROM), linetype = 'dashed', alpha = 0.5) +
  ...
  labs(x = "Position within Gene", colour = "Alelle", title = "Allelic Imbalance (DNA) of Class I Alleles",
...
write.table(hlahat_types_tab, paste0(sid, '_hlatypes.tsv'), sep='\t', quote=FALSE)
```
...
write.table(dna_alleleld_tab, paste0(sid, '_dna_alleleld.tsv'), sep='\t', quote=FALSE)
```
...
write.table(rna_alleleld_tab, paste0(sid, '_rna_alleleld.tsv'), sep='\t', quote=FALSE)
